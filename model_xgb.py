...
def merge_batches(dataloader: DataLoader, is_single_label: bool):
    ...
    for x, y in dataloader:
        ...
        if is_single_label:
            # for the multi-class, single-label tasks, we need to change
            # the ohe encoding to raw labels for input to training
...
class Model:
    ...
    def train(
        ...
        ****************************************************************************
        ...
        # If PyTorch dataloader for training set doen't already exists, get
        ...
        # Training (no loop)
        x_train, y_train = merge_batches(
            self.trainloader, (self.task_type == "single-label")
        )
        print(x_train.shape, y_train.shape)
        if val_dataset:
            valloader = self.get_dataloader(val_dataset, self.test_batch_size, "test")
            x_valid, y_valid = merge_batches(
                valloader, (self.task_type == "single-label")
            )
        else:
        ...
        logger.info(
            "{:.2f} sec used for xgboost. ".format(train_duration)
    ...
    def test(self, dataset, remaining_time_budget=None):
        ...
        # If the task is multi-class single label, the output will be in
        # raw labels; we need to convert to ohe for passing back to
        # ingestion
